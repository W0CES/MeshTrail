"""Verify that a release tag matches every MeshTrail version declaration."""
...
def check_release_version(tag: str, root: Path) -> str:
    ...
    init_text = (root / "meshtrail_plugin" / "__init__.py").read_text(encoding="utf-8")
    init_match = re.search(r'^__version__\s*=\s*["\']([^"\']+)["\']', init_text, re.MULTILINE)
    if init_match is None:
        raise ValueError("meshtrail_plugin.__version__ could not be read")

    declared = {
        "pyproject.toml": str(project["project"]["version"]),
        "openhop-plugin.json": str(manifest["version"]),
        "meshtrail_plugin/__init__.py": init_match.group(1),
    }
