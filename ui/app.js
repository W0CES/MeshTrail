(() => {
  ...
  const PLUGIN_ID = "openhop.meshtrail";
  ...
  async function apiFetch(url, options = {}) {
    ...
    if (response.status === 401) {
      throw new Error(
        token
          ? "Your openHop dashboard session expired. Sign in again and reopen MeshTrail."
          : "Open MeshTrail from the signed-in openHop dashboard."
      );
    }
    return response;
  }
  ...
  $("settings-form").addEventListener("submit", async (event) => {
    ...
    try {
      ...
      setNotice("Saving settings and restarting MeshTrail...");
      ...
      setNotice("Saved. MeshTrail is restarting with the new trail limits.", "ok");
